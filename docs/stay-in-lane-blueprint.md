...
| **Dataset** | 50 `researched`. `--holes` is AR and SD private duty. Census: `docs/state-dataset-blueprint.md` and playbook Track 3. | Ship-quality, not a category change. Confirm with sources; do not invent fields to look complete. Do not mix one state's list into another. |
