...
Matrix edition is **2026-09-24**. App version **v2.9.53**. The log,
...
| **3. Holes when you want completeness** | Batches A–E done 2026-08-14. 2026-08-18 hole-close: MS Chapter 09 researched/`required`; MN, MI, VA, SC, KS `privateDuty: none` from exclusive who-clauses. 2026-09-24 primary-text audit of all 50: AR, DE, GA, ID, IL, MO, MT, NC, OR `privateDuty: none` from quoted who-clauses; 15 states `privateDutyScope: rupOnly`. Remaining `--holes`: SD (`researched`/`uncertain`). Promote only from a primary source. Freeze until that URL’s hash changes. | Unfinished research is not maintenance. Re-reading SD every quarter is how this stays expensive. |
