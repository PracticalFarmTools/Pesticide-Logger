...
| Feature | Details |
|---|---|
| **Dynamic per-state / class spray log** | First-run asks **which record list this book uses** (My crop on my land vs this state’s commercial record list), then the log reshapes by state **and** class (`private` / `commercial` / `both`). Private duty is scoped (`required` / `none` / `uncertain`) so Iowa growers stay quiet and Maine growers still get that state’s private list. Conditional fields appear only when applicable. |
...
| **Inspector view & REI board** | Optional shop view hides editing so you can hand the tablet over — Exit anytime, optional PIN, farm name recovers a forgotten PIN. **Print today’s REI board** for the shop door (not the official WPS sign). **Print WPS application info** (English/Spanish): the 40 CFR 170.311(b) facts for every spray still inside its display window, from the records already entered — not WPS compliance software. |
...
| **Clerk tools paper cannot keep** | Home names keep-until year, incomplete, and overdue. Reports prints a **season binder** (not an agency form). CSV import says drafts are incomplete on purpose. |
...
(matrix edition: 2026-09-24; each state also has a `reviewedAt` date in
...
how.html                   Restore, catch up, Add to Home Screen (no service worker)
...
farm-file.js               Crew, gather/merge receipt, signed inspector HTML, REI board, WPS info sheet
