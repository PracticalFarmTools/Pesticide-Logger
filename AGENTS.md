...
- `node --check app.js`, `node --check state_pesticide_laws.js`, `node --check deadline.js`, and `node --check sw.js` provide source syntax checks. Run `node tests/compliance.test.js` for 50-state dataset / deadline / regression checks. There is no configured lint command.
