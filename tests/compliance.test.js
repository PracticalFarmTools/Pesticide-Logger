...
check('source files advertise v2.9.44 + deadline/license wiring', () => {
  const app = fs.readFileSync(path.join(root, 'app.js'), 'utf8');
  const sw = fs.readFileSync(path.join(root, 'sw.js'), 'utf8');
  const html = fs.readFileSync(path.join(root, 'index.html'), 'utf8');
  assert.ok(app.includes('v2.9.44'));
  assert.ok(sw.includes('pesticide-logger-v2.9.44'));
  assert.ok(sw.includes("const LAWS_EDITION = '2026-09-15'"));
  assert.ok(!html.includes('v2.9.44'), 'version stays out of the header and About copy');
...
check('cab UX: compact spray log, library-first lists, quieter home, calc copy, map default', () => {
  ...
  assert.ok(app.includes("const APP_VERSION = 'v2.9.44'"));
  assert.ok(!html.includes('v2.9.44'));
});
...
check('share plays: public page, generic CSV chooser, restore card, one-pagers', () => {
  ...
  assert.ok(!start.includes('v2.9.44'), 'public page keeps version out of copy');
