...
check('tiny farm: no extra chrome', () => {
  ...
  assert.strictEqual(FS.shouldQuietHome(tinyFields.length, tinyApps.length), true);
  ...
  assert.strictEqual(FS.shouldQuietHome(8, 5), false);
  assert.strictEqual(FS.shouldQuietHome(2, 20), false);
});
