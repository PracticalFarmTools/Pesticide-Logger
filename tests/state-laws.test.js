...
check('sw cache name splits app version from laws edition', () => {
  ...
  assert.ok(sw.includes("const APP_CACHE = 'pesticide-logger-v2.9.49'"));
  assert.ok(sw.includes("const LAWS_EDITION = '2026-09-24'"));
  assert.ok(sw.includes("const CACHE_NAME = APP_CACHE + '-laws-' + LAWS_EDITION"));
  assert.ok(!sw.includes("const CACHE_NAME = 'pesticide-logger-v2.9.49';"));
});
...
check('dataset census: verification and privateDuty buckets', () => {
  ...
  assert.deepStrictEqual(duty.none, ['AL', 'AR', 'DE', 'GA', 'ID', 'IL', 'IA', 'KS', 'MI', 'MN', 'MO', 'MT', 'SC', 'VA']);
