...
- `node --check app.js`, `node --check store.js`, `node --check compliance.js`, `node --check camera-scan.js`, `node --check sw.js` provide source syntax checks. Run `node tests/compliance.test.js`, `node tests/store.test.js`, `node tests/compliance-engine.test.js`, and `node tests/camera-scan.test.js` for dataset / persistence / compliance / camera-path checks. There is no configured lint command.
