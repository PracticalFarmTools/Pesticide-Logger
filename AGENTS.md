...
- A legal change for one state is `laws/XX.json` plus `node tools/bundle-state-laws.js`. That must not require edits to `app.js` or `compliance.js`. `node tools/bundle-state-laws.js --holes` lists unfinished dataset rows; `--oldest 13` is the quarterly re-read queue.
