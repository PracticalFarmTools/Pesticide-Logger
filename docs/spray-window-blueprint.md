...
**Status: implemented** in `spray-window.js` + the dashboard card / field mapper pin.
...
README currently said “per mapped field.” That was the intended product, not the old single-cache behavior. The implementation stores one cache per field pin in `data.meta.forecastByField`.
...
Phases 1–3 are in this change. Phase 4 stays out until the field-tested chart is trusted.
