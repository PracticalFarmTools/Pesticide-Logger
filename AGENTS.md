...
- This repository is one static, offline-first PWA with no backend, package manager, build step, or automated test suite. The standard local server command is documented in `README.md`.
...
- `node --check app.js`, `node --check state_pesticide_laws.js`, and `node --check sw.js` provide the available source syntax checks; there is no configured lint command.
