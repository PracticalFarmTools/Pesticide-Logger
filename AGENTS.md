...
- `node --check app.js`, `node --check state_pesticide_laws.js`, `node --check deadline.js`, `node --check farm-scale.js`, and `node --check sw.js` provide source syntax checks. Run `node tests/compliance.test.js` and `node tests/farm-scale.test.js` for 50-state dataset / deadline / farm-scale / regression checks. There is no configured lint command.
